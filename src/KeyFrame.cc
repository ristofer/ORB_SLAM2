...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap), mpNextKeyFrame(NULL),mpPreviousKeyFrame(NULL),isWorldFrame(false)
{
    mnId=nNextId++;
...
void KeyFrame::save(Archive &ar, const unsigned int version) const
{
    ...
    {
        boost::mutex::scoped_lock lock_pose(mMutexPose);
        ar & Tcw & Twc & Ow & Cw;
    }
    {
        boost::mutex::scoped_lock lock_feature(mMutexFeatures);
    ...
    {
        // Grid related
        boost::mutex::scoped_lock lock_connection(mMutexConnections);
...
void KeyFrame::load(Archive &ar, const unsigned int version)
{
    ...
    // BoW
    ar & mpKeyFrameDB;
    // mpORBvocabulary restore elsewboost::mutex::scoped_lock lock_connection(mMutexConnections);here(see SetORBvocab)
    {
        // Grid related
